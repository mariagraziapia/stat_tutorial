...
coeff<-c(1.02, 1.02, 1.02, 1.02, 1.02, 1.02, 1.01, 1.01, 1.01, 1.01, 1.01)
...
lines(xPlot, g2Plot, col="red", lwd=2, lty=1)
# Points on the gaussians
points(x,g1)
points(x,g2, pch=15, col="red")
# Error bars
segments(x, g1+errors, x, g1-errors)

pv<-c(wil$p.value, bar$p.value, difsig$p.value, kru$p.value, ks$p.value, pvchi2)
options(scipen=100)
names(pv)<-c("Wilcoxon signed rank", "Bartels", "Diffsign", "Kruskal-Wallis", "KS", "chi2")
print(pv)
